...
(function ($)
{
    $.fn.sortableLists = function (options)
    {
            ...
            if (state.isDragged)
            {
                ...
                if (doc.scrollTop() > state.rootEl.offset.top - 10 && e.clientY < 50)
                {
                    ...
                    } else
                    {
                        ...
                        jQuery('html, body').each(function (i)
                        {
        ...
        function tidyEmptyLists()
        {
            ...
            jQuery(setting.listSelector, state.rootEl.el).each(function (i)
            {
...
function MenuEditor(idSelector, settings) {
    ...
    var iconPicker = jQuery('#mnu_iconpicker').iconpicker(iconPickerOpt);
    iconPicker.on('change', function (e) {
        jQuery("#mnu_icon").val(e.icon);
    });
    var itemEdit = 0;
    ...
    function createMenu (arrayItem, depth) {
        ...
        if (level === 0) {
            ...
            $elem = jQuery('<ul class="dropdown-menu">');
        }
        $.each(arrayItem, function (k, v) {
            var isParent = (typeof (v.children) !== "undefined") && ($.isArray(v.children));
            var $li = jQuery('<li>');
            // $li.attr('id', v.text);
            $li.data('text', v.text).data('icon', v.icon);
            var $div = jQuery('<a>');
            var $i = jQuery('<i>').addClass('fa ' + v.icon);
            var $span = jQuery('<span>').addClass('txt').append(v.text);

            $div.attr('href', v.href).append($i).append("&nbsp;").append($span);
            $li.append($div);
            if (isParent) {
    ...
    function TButton(attr) {
        return jQuery("<a>").addClass(attr.classCss).attr("href", "#").text(attr.text);
    }
    ...
    function setOpeners(){
        $main.find('li').each(function (){
            ...
            if (li.children('ul').length){
                li.children('a').first().addClass('dropdown-toggle').attr('data-toggle','dropdown')
